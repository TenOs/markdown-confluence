...
Fenced code blocks tagged `plantuml`, `puml`, or `uml` are rendered to PNG via a PlantUML server and uploaded as page attachments. The plugin defaults to the public server at `https://www.plantuml.com/plantuml`, which means **diagram source is sent to a third party**. For private diagrams, run a local PlantUML server (`docker run -d -p 8080:8080 plantuml/plantuml-server:jetty`) and set the URL in the plugin's settings tab. PlantUML rendering can be turned off entirely with the "Enable PlantUML rendering" toggle.
